...
- **Moderation:** Server **GetObject** from S3 → **sharp** transcode to **JPEG** → **Rekognition** `DetectModerationLabels` on **image bytes** (avoids Rekognition rejecting WebP from S3). At or above 80% confidence on configured explicit nudity / graphic violence categories → treat as failed: S3 object deleted, row `FAILED_MODERATION` + `deleted_at`.
